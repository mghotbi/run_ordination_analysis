...
# Author: Mitra Ghotbi
# Description:
# A flexible ordination and PERMANOVA analysis pipeline for phyloseq objects,
# supporting custom grouping, interactions, and automatic sample filtering.
# Supports dynamic group, interaction, and replicate variables.
#
# Example Usage:
# source("ordination_pipeline.R")
# result <- run_ordination_analysis(ps_obj = ps_16s, marker_label = "Prokaryotic Community", 
#                                   group_var = "Treatment", interaction_var = "Genotype", 
...
# Filter out unwanted samples (keep "Bulk")
  ...
  # Optional: Show how many samples were removed
  removed_samples <- sum(!keep_samples)
  cat(paste("Removed", removed_samples, "samples due to missing or invalid group labels.\n"))
  
  # If no samples remain after filtering, stop gracefully
  ...
  # Run ordination
  result <- process_ordination_general(ps_obj, marker_label, group_var, treatment_levels)
  print(result$plot)
  
  result$meta <- droplevels(result$meta)
...
process_ordination_general <- function(ps_obj, title_label = "Ordination Plot", group_var = "Group", treatment_levels = NULL) {
  ...
  ord_df <- plot_ordination(ps_obj, ord, color = group_var)$data %>%
    mutate(
      SampleID = rownames(.),
  ...
  p <- ggplot(ord_df, aes(x = Axis.1, y = Axis.2, color = GroupVar)) +
    ...
    geom_point(data = centroids, aes(x = PC1, y = PC2, fill = GroupVar, color = GroupVar),
...
# Example
# result_16s <- run_ordination_analysis(
#   ps_obj = ps_16s,
...
# 
# 
# # Example: Fungal Community
# result_its <- run_ordination_analysis(
#   ps_obj = ps_its,
#   marker_label = "Fungal Community",
#   group_var = "Treatment",
#   interaction_var = "Genotype",
#   replicate_var = "Replicate",
#   treatment_levels = c("Control", "Flooding", "Herbivory", "DualStress"))
